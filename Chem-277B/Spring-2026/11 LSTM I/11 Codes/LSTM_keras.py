...
# L = LSTM_keras()
# L.CreateModel(dt = 40, n_neurons = 100)
# L.Predict()
# 
# =============================================================================
###############################################################################
...
def FindMyFile(filename: str, ServerHardDiscPath: str = r"c:\Users\MMH_user\Desktop\Berkeley\MSSE") -> str:
    """
    finds file of name "filename" anywhere in "ServerHardDiscPath" and returns complete path
    """
    for r,d,f in walk(ServerHardDiscPath):
        for files in f:
             if files == filename: #for example "MessyFile.xlsx"
                 file_name_and_path =  path.join(r,files)
